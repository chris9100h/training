                    ...
                    {e.sets.map((st, j) => {
                      ...
                      return (
                        <span key={j} style={{
                          ...
                          {st.kg ?? '—'}<span style={{ color: gold ? UI.goldSoft : UI.inkFaint, fontSize: 10 }}>kg</span><span style={{ color: gold ? UI.goldSoft : UI.inkFaint, margin: '0 1px' }}>×</span>{(st.repsL != null || st.repsR != null) ? `L${st.repsL ?? '?'}/R${st.repsR ?? '?'}` : (st.reps ?? '—')}
                        </span>
                      );
                    })}
                  </div>
