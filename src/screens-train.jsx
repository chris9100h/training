...
// Optional finishers on the last round of a Drop Set / Myo-Rep / Myo-Rep Match /
// AMRAP Variations chain: a partials count and/or a DC-style weighted stretch
// (timed hold at a set weight, its own weight since the stretch load often
// differs from the working set). Collapsed by default (nothing written); tap
// "+ FINISHER" to reveal. Seconds step by 5 (like time exercises); the stretch
// weight starts at the working weight and nudges by the exercise increment.
...
function TrainingScreenInner({ store, setStore, go, sessionId, userId, session, syncStatus, storageFull, onRetrySync }) {
  ...
  // Delegate to the shared rules instead of keeping a second, weaker copy:
  // the local versions handled only kg x reps, so time sets (planks, hangs)
  // and reps-only exercises never got an improvement or regression overlay
  // here, while every other screen scored them correctly.
  // `justCompleted` forces done for the set being evaluated at completion
  // time: the state copy in hand can still be the pre-patch row, and the
  // shared rules require done. Scans over earlier sets pass it through as is,
  // so a skipped set is not counted as an improvement.
  const isImprovement = (st, prevSet, justCompleted = false) =>
    LB.isImprovement(justCompleted ? { ...st, done: true } : st, prevSet);
  const isDecline = (st, prevSet, justCompleted = false) =>
    LB.isDecline(justCompleted ? { ...st, done: true } : st, prevSet);
  ...
  const completeSet = (setIdx, bypassOutlierCheck = false, advanceFocus = false, extraPatch = null) => {
    ...
    if (!bypassOutlierCheck && !entry.sets[setIdx]?.warmup && !_isDeloadSet) {
      ...
      if (!isNoWeightReps && !LB.isAssisted(exercise)) {
        ...
        if (refKg != null && refKg > 0) {
          increment = LB.incrementForExercise(store, exercise, null);
    ...
    if (extraPatch) { setLpTarget(null); setLpCount(0); setLpStretch(null); setWsTarget(null); setWsStretch(null); }
    ...
    const progressionResult = (() => {
      ...
      const increment = LB.incrementForExercise(store, exercise, null, catCfg);
    ...
    if (progressionResult) {
      ...
      if (isNewBest) {
      ...
      } else if (isImprovement(completed, prevSet, true)) {
        setImprovedSet(true);
        overlayHoldMs = FLASH_NAV_DELAY_MS;
        ...
        if (!anyImprovementBefore && isDecline(completed, prevSet, true) && store.settings?.showRegression !== false) {
  ...
  const flashOverlayForCompletedSet = (targetIdx, firstSet) => {
    ...
    if (!entry.sets[targetIdx]?.warmup && !isDeloadSession && firstSet.kg != null && firstSet.reps > 0) {
      ...
      if (isNewBest) {
      ...
      } else if (isImprovement(firstSet, prevSet, true)) {
        ...
        if (!anyImpBefore && isDecline(firstSet, prevSet, true) && store.settings?.showRegression !== false) {
    ...
    updateSession(sess => ({
      ...
      entries: sess.entries.map((en, ei) => ei !== exIdx ? en : {
        ...
        sets: en.sets.map((st, si) => si !== dropSetIdx ? st : {
          ...
          // e1RM and PR detection score this set with the SEED reps instead of
          // what was actually lifted. The session editor's own chain writer
          // already nulls them.
          repsL: null,
          repsR: null,
          done: true,
  ...
  const finishLengthenedPartial = async (setIdx) => {
    ...
    // just the set (or, if a stretch was added, that's the standalone
    // Weighted Stretch technique). FINISH stays tappable even at 0 (see
    // finishDropSet) so this can explain why instead of silently tagging the
    // set with a technique nothing was actually done for.
    if (lpCount === 0) {
    ...
    completeSet(setIdx, false, true, { technique: 'lengthened_partial', drops: lpStretch ? { partials: lpCount, stretch: lpStretch } : { partials: lpCount } });
  };
  ...
  const checkSet = () => {
    ...
    if (wsTarget?.exIdx === exIdx && wsTarget?.setIdx === idx) { finishWeightedStretch(idx); return; }
  ...
  useEffectT(() => {
    if (dropSetIdx != null || myoSetIdx != null || lpTarget != null || wsTarget != null || avSetIdx != null) {
      try {
        localStorage.setItem('logbook-intensity-state', JSON.stringify({
          sessionId, exIdx,
          dropSetIdx, dropDrops,
          myoSetIdx, myoDrops, myoTechnique, myoTarget,
          lpSetIdx: lpTarget?.setIdx ?? null, lpCount, lpStretch,
          wsSetIdx: wsTarget?.setIdx ?? null, wsStretch,
          avSetIdx, avDrops,
        }));
      } catch {}
  ...
  }, [dropSetIdx, dropDrops, myoSetIdx, myoDrops, myoTechnique, myoTarget, lpTarget, lpCount, lpStretch, wsTarget, wsStretch, avSetIdx, avDrops, sessionId, exIdx]);
  useEffectT(() => {
    try {
      ...
      if (st.wsSetIdx != null && !targetEntry?.sets[st.wsSetIdx]?.done) {
        setWsTarget({ exIdx, setIdx: st.wsSetIdx });
        setWsStretch(st.wsStretch || { kg: null, timeSec: 30 });
      }
  ...
  if (mesoBoostSetsInitRef.current === null) {
    ...
    // Rebuild the gate sets EXACTLY like LB.mesoGateSetsFromAnswers. Joint, pump and
    // weight-feel are all per exId now; each resolves per exId with a legacy per-muscle
    // fallback (an old in-progress session reloaded mid-deploy kept pump/weight on the
    // volume rec). Soreness stays per muscle. weightOk is built in every mode now.
    ...
    for (const [exId, rec] of Object.entries(a.joint || {})) {
      if (rec && 'weight' in rec) { weightAnswered.add(exId); if (LB.volumeAnswerAllowsBump(rec.weight, true)) weightOk.add(exId); }
    }
    for (const rec of Object.values(a.volume || {})) {
      if (!rec || rec.volume == null || !LB.volumeAnswerAllowsBump(rec.volume, loadOnly)) continue;
    ...
    mesoBoostSetsInitRef.current = { jointFine, pumpOk, weightOk, soreBlock };
  }
  const mesoJointFineRef = useRefT(mesoBoostSetsInitRef.current.jointFine);    // exIds where joint was 'fine'
  const mesoPumpOkRef = useRefT(mesoBoostSetsInitRef.current.pumpOk);          // exIds whose pump was moderate/amazing
  const mesoWeightOkRef = useRefT(mesoBoostSetsInitRef.current.weightOk);      // exIds whose weight-feel answer allows the bump (every mode)
  ...
  const handleJointAnswer = (answer, weight, pump, affinity) => {
    ...
    // Weight-feel and pump both ride on this per-exercise joint record now, in every
    // mode, and gate the weight boost (weightOk / pumpOk keyed by exId). Neither moves
    // a set delta. Pump also feeds the per-exId low-pump swap counter.
    if (weight != null) {
    ...
    // In Volume+Load and non-final Meso weeks, after the LAST exercise of a muscle
    // group open the per-muscle WORKLOAD sheet (it drives set deltas). Load-only and
    // the Meso final week freeze set deltas, so there is no per-muscle step there at
    // all: pump moved to this per-exercise sheet and the workload answer would do
    // nothing. Unchanged from a fresh ask otherwise: if the muscle's sheet already
    // fired (askedVolumeRef), editing an earlier joint answer just returns.
    if (LB.autoregLoadOnly(mesoSch) || mesoLastWeek) return;
  ...
  const handleVolumeAnswer = (volume) => {
    ...
    // per-exercise step, mesoPumpOkRef keyed by exId). It drives set deltas in
    // Volume+Load / non-final Meso weeks, never the weight gate.
  ...
  // per-muscle set-volume question (Volume+Load / Meso only): how much total work the
  // muscle got, which drives the set deltas. Same storage, different strings.
  const weightFeelMode = LB.autoregLoadOnly(mesoSch);
  const WEIGHT_LABELS = { not_enough: 'Too light', just_right: 'Just right', pushed: 'Hard', too_much: 'Too heavy' };
  ...
  const mesoRecapGroups = () => {
    ...
    muscleOrder.forEach(muscle => {
      ...
      // Per-exercise feedback (joint + weight-feel + pump, all folded into each
      // exercise's row) vs. per-muscle feedback (Soreness carryover + the workload
      // answer, Volume+Load / Meso only), split into two sections in the detail sheet
      // so it's clear which answer applies to what.
      const jointRows = [];
      session.entries.forEach(e => {
        if (e.isCardio) return;
      ...
      // Per-muscle workload drives the set deltas (Volume+Load / non-final Meso weeks).
      // Load-only and the Meso final week freeze sets, so there is no workload answer.
      if (vRec?.volume != null) {
        generalRows.push({ key: 'volume-' + muscle, title: 'Workload', sub: WORKLOAD_LABELS[vRec.volume] || vRec.volume, onEdit: () => openVolumeEdit(muscle) });
      }
  ...
  // drops[dropIdx] of that chain. `field` is 'kg' or 'sec'.
  const readStretch = (target, dropIdx) => (
    target === 'ws' ? (wsStretchRef.current || {})
    : target === 'lp' ? (lpStretchRef.current || {})
  ...
  const setStretchVal = (target, dropIdx, patch) => {
    ...
    if (target === 'lp') { setLpStretch(prev => ({ kg: null, timeSec: null, ...(prev || {}), ...patch })); return; }
  ...
  const kbConfirm = () => {
    ...
    if (field === 'kg') {
      ...
      if (wsTarget?.exIdx === exIdx && wsTarget?.setIdx === setIdx) return;
  ...
  useEffectT(() => {
    ...
    if (curWarmup) return;
    // lpTarget/wsTarget carry their own exIdx. Blocking on them regardless of
    // which exercise they belong to meant that leaving an exercise with an
    // open lengthened-partials or weighted-stretch editor (no FINISH, no
    // CANCEL) silently disarmed planned techniques and meso LP for every
    // REMAINING exercise of the session. Only the exercise that actually owns
    // the open editor should be blocked.
    if (dropSetIdx != null || myoSetIdx != null || avSetIdx != null) return;
    if (lpTarget?.exIdx === exIdx || wsTarget?.exIdx === exIdx) return;
  ...
  // Skipped sets are finished business, exactly like done ones: without the
  // !s.skipped test, coming back to an exercise after "Skip remaining sets"
  // put the hero card on a skipped set and let it be edited, while its own
  // list row stayed locked and the footer button stayed disabled. Anything
  // typed there never counted toward volume or PRs. Same predicate the three
  // other "next open set" scans in this file use.
  const currentSetIdx = entrySets.findIndex(s => !s.done && !s.skipped);
  ...
  return (
    <Screen scroll={false}>
          ...
          <div style={{ background: UI.bg, backgroundImage: 'var(--bg-texture)', borderRadius: '6px 6px 0 0', borderTop: `var(--hair-width) solid ${UI.hairStrong}`, width: '100%', maxWidth: 480, padding: '20px 20px 44px' }}>
            ...
            <div style={{ display: 'flex', gap: 10 }}>
              ...
              <Btn style={{ flex: 1 }} onClick={() => {
                const s = outlierConfirm.setIdx;
                setOutlierConfirm(null);
                const finExtra = (lpTarget?.exIdx === exIdx && lpTarget?.setIdx === s)
                  ? { technique: 'lengthened_partial', drops: lpStretch ? { partials: lpCount, stretch: lpStretch } : { partials: lpCount } }
                  : (wsTarget?.exIdx === exIdx && wsTarget?.setIdx === s)
                  ? { technique: 'weighted_stretch', drops: { stretch: wsStretch } }
                  : null;
                completeSet(s, true, true, finExtra);
              }}>{(() => {
                const oc = outlierConfirm;
              ...
              return (
                <React.Fragment key={i}>
                  ...
                  {(() => {
                    ...
                    const isWsActive = wsTarget?.exIdx === exIdx && wsTarget?.setIdx === i && !s.done;
                    ...
                    <div data-kb-row={i} style={{
                      ...
                      {isIntensityActive ? null : isTime ? (
                        ...
                        (s.technique === 'drop' || s.technique === 'myorep' || s.technique === 'myorep_match' || s.technique === 'lengthened_partial' || s.technique === 'weighted_stretch' || s.technique === 'amrap_variations') && s.done
                  ...
                  {lpTarget?.exIdx === exIdx && lpTarget?.setIdx === i && !s.done && (() => {
                    ...
                    const lpS = lpStretch || { kg: null, timeSec: 30 };
                    const lpKgActive = kbField?.setIdx === 'stretch' && kbField?.target === 'lp' && kbField?.field === 'kg';
                    const lpSecActive = kbField?.setIdx === 'stretch' && kbField?.target === 'lp' && kbField?.field === 'sec';
                    const closeLpStretchKb = () => {
                      if (kbFieldRef.current?.setIdx === 'stretch' && kbFieldRef.current?.target === 'lp') {
                        kbFieldRef.current = null; kbRawRef.current = ''; kbFreshRef.current = false;
                        setKbField(null); setKbRaw('');
                      }
                    };
                    return (
                      <div style={{ marginLeft: 36, paddingLeft: 10, borderLeft: `2px solid rgba(var(--accent-rgb),0.3)` }}>
                        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '6px 4px 2px' }}>
                          ...
                          <button onClick={() => { setLpTarget(null); setLpCount(0); setLpStretch(null); closeLpStretchKb(); }} style={{ background: 'none', border: 'none', color: UI.inkFaint, fontSize: 10, fontFamily: UI.fontUi, cursor: 'pointer', padding: '2px 4px', letterSpacing: '0.08em' }}>CANCEL</button>
                        </div>
                        ...
                        {/* Optional finisher on top of the partials: squeeze out
                            as many partials as possible, then hold the stretch
                            once the weight won't move anymore. Chip-reveal like
                            the chain-sheet Finisher; once added, same boxed
                            ...
                            <div style={{ display: 'flex', alignItems: 'flex-end', gap: 16, justifyContent: 'flex-end', flexWrap: 'wrap' }}>
                              {stretchShowWeight && (
                                <div style={{ display: 'flex', flexDirection: 'column', gap: 3, alignItems: 'center' }}>
                                  <span className="micro" style={{ color: UI.inkFaint }}>WEIGHT</span>
                                  <div style={{ width: 72 }}>
                                    <KbCell text={lpKgActive ? kbRaw : (lpS.kg != null ? String(lpS.kg).replace('.', ',') : '')} placeholder="—" onActivate={() => activateStretchKb('lp', null, 'kg')}
                                      style={{ ...setInputStyle(false, lpKgActive), ...(lpKgActive ? { boxShadow: 'inset 0 -2px 0 var(--accent)' } : {}) }} />
                                  </div>
                                </div>
                              )}
                              <div style={{ display: 'flex', flexDirection: 'column', gap: 3, alignItems: 'center' }}>
                                <span className="micro" style={{ color: UI.inkFaint }}>HOLD·S</span>
                                <div style={{ width: 56 }}>
                                  <KbCell text={lpSecActive ? kbRaw : (lpS.timeSec != null ? String(lpS.timeSec) : '')} placeholder="—" onActivate={() => activateStretchKb('lp', null, 'sec')}
                                    style={{ ...setInputStyle(false, lpSecActive), ...(lpSecActive ? { boxShadow: 'inset 0 -2px 0 var(--accent)' } : {}) }} />
                                </div>
                              </div>
                            </div>
                          </div>
                      ...
                      <div data-stretch-box="ws" style={{ background: 'rgba(var(--accent-rgb),0.11)', borderRadius: 6, margin: '2px 0 6px' }}>
                        ...
                        <div style={{ padding: '0 6px 8px' }}>
                          ...
                          <button onClick={() => finishWeightedStretch(i)}
                            ...
                            style={{
                              width: '100%', padding: '8px 0',
                              background: !missingData ? 'rgba(var(--accent-rgb),0.12)' : 'transparent',
                              border: `1px solid ${!missingData ? 'rgba(var(--accent-rgb),0.5)' : UI.hair}`,
                              borderRadius: 6, color: !missingData ? 'var(--accent)' : UI.inkGhost,
                              fontFamily: UI.fontUi, fontSize: 10, fontWeight: 700, letterSpacing: '0.1em',
                              cursor: missingData ? 'default' : 'pointer',
                              WebkitTapHighlightColor: 'transparent',
                            }}>✓ FINISH</button>
                  ...
                  {s.technique === 'lengthened_partial' && s.done && !s.warmup && (s.drops?.partials || 0) > 0 && (
                    <div style={{ marginLeft: 36, paddingLeft: 10, paddingBottom: 8 }}>
                      <div style={{ display: 'inline-flex', alignItems: 'center', gap: 6, padding: '3px 8px', border: '1px solid var(--accent)', borderRadius: 4, fontFamily: UI.fontUi, fontSize: 11, color: 'var(--accent)', letterSpacing: '0.03em' }}>
                        <span>{s.drops.partials} partial{s.drops.partials === 1 ? '' : 's'}</span>
                        {s.drops.stretch && <span style={{ opacity: 0.4 }}>|</span>}
                        {s.drops.stretch && <span>stretch {s.drops.stretch.kg != null ? String(s.drops.stretch.kg).replace('.', ',') + ' ' + UI.unit() + ' · ' : ''}{s.drops.stretch.timeSec}s</span>}
                      </div>
                    </div>
                  )}
                  ...
                  {s.technique === 'weighted_stretch' && s.done && !s.warmup && s.drops?.stretch && (
                    <div style={{ marginLeft: 36, paddingLeft: 10, paddingBottom: 8 }}>
                      <div style={{ display: 'inline-block', padding: '3px 8px', border: '1px solid var(--accent)', borderRadius: 4, fontFamily: UI.fontUi, fontSize: 11, color: 'var(--accent)', letterSpacing: '0.03em' }}>
                        stretch {s.drops.stretch.kg != null ? String(s.drops.stretch.kg).replace('.', ',') + ' ' + UI.unit() + ' · ' : ''}{s.drops.stretch.timeSec}s
                      </div>
                    </div>
                  )}
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
              ...
              <button onClick={() => {
                const target = currentSetIdx >= 0
                  ...
                  : entry.sets.reduce((last, s, i) => !s.warmup ? i : last, -1);
                if (target < 0) return;
                clearDrop(); clearMyo(); clearWs(); clearAv();
              ...
              <button onClick={() => {
                const target = currentSetIdx >= 0
                  ...
                  : entry.sets.reduce((last, s, i) => !s.warmup ? i : last, -1);
                if (target < 0) return;
                clearDrop(); clearMyo(); clearLp(); clearAv();
                const s = entry.sets[target];
      ...
      {/* plan diff */}
      {/* Dismissing goes BACK to the finish flow, it does not silently pick a
          side. This used to run the "Leave plan" branch: a backdrop tap ended
          the session immediately and threw away both the offered plan update
          and the rep-target wizard, the more destructive of the two choices,
          with no way to get the prompt back. */}
      <Sheet open={planDiffOpen} onClose={() => setPlanDiffOpen(false)} title="Session changes">
      ...
      {/* Per-muscle workload (Volume+Load / non-final Meso weeks): drives the set deltas.
          Pump moved to the per-exercise sheet; load-only / meso final week never open this. */}
      <Sheet open={mesoVolumeOpen} onClose={() => {}} title={mesoVolumeMusc ? `${mesoVolumeMusc} workload` : 'Workload'}>
        <div style={{ fontSize: 12, color: UI.inkFaint, fontFamily: UI.fontUi, marginBottom: 4, letterSpacing: '0.06em', textTransform: 'uppercase' }}>Workload</div>
        ...
        <Btn
          disabled={!mesoVolumeAnswer}
          onClick={() => handleVolumeAnswer(mesoVolumeAnswer)}
          style={{ width: '100%' }}
        >
