...
async function loadFromSupabase(userId, _depth = 0, _opts = {}) {
  ...
  const queries = [
    ...
    isCoachLoad ? null : _supabase.from('zane_medications').select('id, name, brand, category, unit_label, package_size, stock_baseline, stock_set_at, archived, exclude_from_pillbox, low_stock_threshold, exclude_from_low_stock, created_at, updated_at').eq('user_id', userId),
  ...
  const result = {
    ...
    medications: (medicationsRes?.data || []).map(m => ({
      ...
      excludeFromLowStock: !!m.exclude_from_low_stock,
...
function mapFoodLogRow(l) {
  return {
    ...
    // used instead of guessing via a matching favorite's first unit.
    loggedUnit: l.logged_unit ?? null,
    // {id, removedEntries} when this entry was created by "split into
    // multiple meals" (screens-food.jsx); lets the split be undone from the
    // timeline any time later, not just the 6s toast right after applying
    // it. Null for an entry logged directly.
    splitBatch: l.split_batch ?? null,
...
async function refreshHealthLogs(userId) {
  ...
  const [dailyRes, cardioRes, glucoseRes, bpRes, tempRes, waterRes, foodRes,
    ...
    _supabase.from('zane_medications').select('id, name, brand, category, unit_label, package_size, stock_baseline, stock_set_at, archived, exclude_from_pillbox, low_stock_threshold, exclude_from_low_stock, created_at, updated_at').eq('user_id', userId),
