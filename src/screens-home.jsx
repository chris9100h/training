...
function HomeScreen({ store, setStore, go, userId, syncStatus, storageFull, onRetrySync }) {
  ...
  // grid canvas needs a touch more mark to stay visible against it, but like
  // dark/black that bump only applies while the grid toggle is on. Dark/black
  ...
  // had. Paper's own grid-off value follows the same 4% floor.
  ...
  const watermarkOpacity = watermarkOpacityOverride != null
    ...
    : (isCustomBg ? 0.16 : (isPaperMode ? (gridOn ? 0.16 : 0.04) : (isLightMode ? 0.14 : (gridOn ? 0.12 : 0.04))));
  ...
  return (
    <Screen scroll={false} style={{ position: 'relative' }}>
      ...
      <div style={{
        flexShrink: 0,
        // +40px, not the original +12px: this is the actual header shown in
        ...
        // fallback a few lines up and was NOT what needed changing). Large,
        // deliberately obvious jump this round instead of another small
        // nudge, so on-device testing can tell at a glance whether pushing
        // this header down at all moves the needle, confirm before tuning
        // the exact number down.
        padding: `calc(env(safe-area-inset-top, 0px) + 40px) 22px 0`,
