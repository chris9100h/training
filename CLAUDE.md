...
**⚠️ OFFENE MIGRATION — Health-Tab (noch anzulegen + auszuführen):** Der App-Code für den Health-Tab ist bereits live, die DB-Änderung fehlt aber noch. Vor dem ersten echten Nutzen anlegen (nächste freie Nummer, z.B. `supabase/migrations/0072_daily_logs.sql`), `supabase/schema.sql` mitziehen und ausführen:
1. **Tabelle `zane_daily_logs`** (Spalten siehe oben) mit `UNIQUE (user_id, date)`.
2. **RLS**: `user_id = auth.uid()` für own CRUD; zusätzlich SELECT für den Coach (Zeile in `zane_coaching` mit `coach_id = auth.uid()` und `client_id = zane_daily_logs.user_id`, `status = 'active'`) — analog zu den bestehenden Coach-Read-Policies auf `zane_sessions`/`zane_sets`.
3. **`zane_user_settings`** um `macro_targets jsonb` und `show_health_tab boolean default false` erweitern.
   Bis dahin: `loadFromSupabase` toleriert die fehlende Tabelle/Spalten (leere Liste / null), der Tab ist per Default aus.
