...
- **localStorage-Keys** (einige Settings liegen parallel im localStorage für schnellen Zugriff vor dem Store-Load; bestehende Keys konsistent halten):
  ...
  - `logbook-shopping-name-overrides`: jsonb-Objekt `{ [foodId]: overrideName }`, User-Rename einzelner Shopping-List-Einträge (Tap auf eine Zeile mit `foodId`), für OFF/USDA-Produkte mit zu generischem Namen (z.B. "Original" bei Marke "Weetabix"). Persistent über `foodId`, gilt für jede zukünftige Liste, kein wöchentliches Neu-Setzen nötig. Nur Zeilen mit echter `foodId` sind umbenennbar (Rezept-Zutaten/Custom Items ohne `foodId` bleiben untappable), kein User-Setting, per Gerät
  - `logbook-shopping-exclusions`: jsonb-Objekt `{ [foodId]: true }`, User-Ausschluss einzelner Shopping-List-Einträge über eine Checkbox links neben jeder Zeile (Default an), für Foods die zwar nach Frequenz/Plan qualifizieren aber kein Grocery-Haul-Item sind (z.B. Whey, alle paar Monate woanders bestellt). Ein entfernter Haken nimmt das Item nur aus Export und Screenshot raus, versteckt es aber nicht: die Zeile bleibt (gedimmt, Checkbox weiter tappbar) sichtbar und rutscht unten in einen eigenen, per Knurl abgetrennten "Excluded"-Abschnitt derselben Liste, so bleibt präsent, dass der Posten weiterläuft und irgendwann nachgekauft werden muss. Erneutes Antippen der Checkbox holt die Zeile zurück nach oben. Nur Zeilen mit echter `foodId` haben eine aktive Checkbox (Rezept-Zutaten/Custom Items ohne `foodId` zeigen sie nur gedimmt/inaktiv), kein User-Setting, per Gerät
