...
**`get_all_users_admin()`** → `TABLE(user_id uuid, name text, email text, sw_version text, created_at timestamptz, approved boolean)` — alle registrierten Accounts (nur Admin, hardcoded auf `office@btc-prime.biz`, unabhängig vom `active_users`-Feature-Grant), für den „All users"-Screen im Admin-Sheet (Settings): Suche nach Name/Email + Filter „nur veraltete Version" (Client vergleicht `sw_version` gegen die eigene, per Cache-Storage ermittelte Version). Deckt — anders als „Recent sign-ups" (nur letzte 50 Neuregistrierungen) und „Active users" (nur gerade/kürzlich trainierende) — jeden Account ab, unabhängig von Aktivität. Migration 0123.
