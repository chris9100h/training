...
- **`docs/internals.md`**: Deep-Dives zu Precompile-Loader, System-Übungskatalog (`exercise-db.js`), Plan-Setup-Wizard, History-Windowing/Boot-Merge, 5/3/1, Time-based/Assisted Exercises, Keyboard/Viewport-Geometrie, plus die Verhaltens-Contracts aller localStorage-Keys. Vor Änderungen an diesen Bereichen lesen.
...
- **Dateistruktur:**
  ...
  - `supabase/`: Migrationen, Edge Functions, Schema. **`supabase/functions/_shared/`** hält den geteilten Edge-Function-Code. Ordner mit `_`-Präfix werden **nicht** als eigene Function deployed, die CLI bündelt sie in jeden Importeur: eine Änderung dort wird erst wirksam, wenn **alle Importeure neu deployed** werden. Kein Check erkennt das, `tools/check-edge-functions.cjs` parst nur die Syntax. Vor jeder Änderung in `_shared/` deshalb die Importeur-Liste frisch ermitteln (`grep -rl "_shared/<datei>" supabase/functions/`) und **alle** Treffer deployen. Stand jetzt:
    - `edge.ts` (CORS, JSON-Response, `resolveUser`, `withinQuota`) und `ai.ts` (Anbieter-Tabelle Claude/Grok/Qwen plus Antwort-Helfer): `parse-meal`, `scan-label`, `ai-daily-summary`, `ai-checkin-opinion`.
    - `notifications.ts` (wählt Pushover **oder** Web-Push je Nutzer-Einstellung und meldet zurück, ob der Versand angenommen wurde): `reminder`, `water-reminder`, `meal-reminder`, `daily-log-reminder`, `medication-reminder`, `auto-close-sessions`, `zane_coaching-notify`, `zane_social-notify`. Das sind alle Functions, die eine Nudge-**Entscheidung** treffen, ein Fix hier ist also erst nach **acht** Deploys überall wirksam. (Die Liste stand bis 2026-08-17 auf sieben und hat `zane_social-notify` unterschlagen, obwohl es `sendNotification` importiert. Bei Änderungen an `_shared/` die Importeure immer frisch per `grep -rl` ermitteln, statt dieser Aufzählung zu vertrauen.) **Nicht** dabei sind die drei Transport-Functions `web-push`, `pushover` und `pushover-verify`: die verschicken selbst und werden von `notifications.ts` aufgerufen bzw. direkt vom Client, tragen den Helper aber nicht.
    - `time.ts` (lokale Zeit/Datum je Nutzer, primär über die IANA-Zeitzone, der gespeicherte Minuten-Offset ist nur Legacy-Fallback): `water-reminder`, `meal-reminder`, `daily-log-reminder`, `medication-reminder`.
    - Die übrigen Functions tragen bewusst eigene Kopien von `resolveUser`/`withinQuota` weiter (eine Migration hieße Deploy ohne Verhaltensgewinn) und ziehen einzeln nach, wenn sie ohnehin mal angefasst werden.
...
- **Platz für die Bildschirmtastatur** in einem `position:fixed`-Overlay **nie selbst ausrechnen**, sondern `keyboardViewportReservation()` (`ui.jsx`) nutzen. Die Tastaturhöhe ist nicht die zu reservierende Höhe: Chrome/Android verkleinert wegen `interactive-widget=resizes-content` den Layout-Viewport schon selbst (reservieren = Panel fliegt oben aus dem Bild), iOS nicht (nicht reservieren = Feld liegt hinter der Tastatur). Hintergrund und Regressionsgeschichte: `docs/internals.md`, „Keyboard & Viewport".
...
- **CI-Gates** (`npm run build` plus dreizehn Checks laufen via `.github/workflows/check.yml` bei jedem Push): `tools/check-syntax.cjs` (transpiliert alle Quellen exakt wie der In-App-Loader; die JSX-Liste wird aus `SOURCES` in `index.html` geparst, neue Dateien sind damit automatisch abgedeckt), `npm run build` (erzeugt die `dist/`-Ausgabe fürs Cloudflare Preview, siehe „Architektur") gefolgt von `tools/check-build.cjs` (validiert `dist/`: Bundle-Dateien vorhanden, in `dist/sw.js`-ASSETS gelistet, `PRECOMPILED_BUILD`-Marker gesetzt), `tools/check-edge-functions.cjs` (transpiliert alle Edge-Function-TypeScript-Dateien als Syntax-Check, keine Deno-Laufzeit), `tools/test/reminder-time.test.cjs` und `tools/test/notification-delivery.test.cjs` (Unit-Tests für Reminder-Zeitzonen-Logik bzw. Push-Zustellung), `tools/test/store.test.cjs` (Store-Kernlogik), `tools/test/keyboard-viewport.test.cjs` (Keyboard-Platzreservierung für Fixed-Overlays, siehe „Keyboard & Viewport"), `tools/check-db-docs.cjs` (Migrationen vs. `schema.sql`/`docs/database.md`), `tools/check-backup-coverage.cjs` (export→import-Sandbox, jede Schema-Spalte muss round-trippen, druckt bei Fehlern einen fertigen Fix-Prompt), `tools/check-cache-version.cjs` (`?v=`-Buster der Public-Seiten vs. SW-Cache-Version), `tools/check-emdash.cjs` (einzige Ausnahme: die alleinstehende Platzhalter-Glyphe), `tools/check-asset-parity.cjs` (alles, was `index.html` lädt, muss in `ASSETS` des SW stehen; Public-Seiten bleiben draußen), `tools/check-sw.cjs` (führt `sw.js` in einer Node-VM aus: Branch-Routing, Offline-Fallbacks, waitUntil-Abdeckung), `tools/check-viewport-geometry.cjs` (AST-Scan: Overlays, die sich am Layout- statt am sichtbaren Viewport bemessen, siehe „Keyboard & Viewport" in `docs/internals.md`; Allowlist mit Begründung im Tool selbst). Wöchentlich vergleicht zusätzlich `tools/check-db-live.cjs` (`db-drift.yml`) die echte DB gegen Snapshot und Doku. **Postet der Nutzer einen fehlgeschlagenen Drift-Lauf, sofort nach dem Playbook in `docs/database.md` („Drift-Checks") bereinigen.**
...
- **localStorage-Keys** (einige Settings liegen parallel im localStorage für schnellen Zugriff vor dem Store-Load; bestehende Keys konsistent halten. Verhaltens-Contracts je Key: `docs/internals.md`, Abschnitt „localStorage-Keys"):
  - `logbook-accent-color` / `logbook-push-enabled` / `logbook-cycle-week-view` (Settings-Spiegel) · `logbook-whatsnew-seen` (zuletzt gesehene `WHATS_NEW.id`) · `logbook-health-card-order` · `logbook-seen-signups` (Admin-Feed-Dismiss, max. 1.000 IDs) · `logbook-dismissed-sessions` (Spectator-Dismiss, max. 200 IDs)
  ...
  - `logbook-food-fav-cache-attempt` / `-repaired` (Favoriten-Cache-Reparatur: Fehlschlag-Datum bzw. erledigte foodIds, verhindern Quota-fressende Retry-Loops; `-repaired` wird auf aktuelle Favoriten gekürzt) · `logbook-shopping-list-days` (Einkaufs-Horizont, Default 7) · `logbook-low-stock-acked` / `logbook-med-low-stock-acked` (weggetippte Running-Low-Banner, an `stock_set_at` gekeyt, je max. 500 IDs) · `logbook-cooking-draft` (In-Progress-Cooking-Mode, nur ein aktiver Cook, überlebt App-Kill) · `logbook-fasting-state` (Intermittent-Fasting-Zyklus-Zustand, per Gerät) · `logbook-meso-asked-*` (Crash-resistente Session-Fragen, maximal 32 Merker plus laufende Session)
...
- Der persistierte localStorage-Store ist ein begrenzter Offline-Arbeitssatz, kein Archiv: Session-Sets 70 Tage, ausführliche Meso-Recaps 730 Tage, Food-/Medikamentenlogs und ihre Marker 30 Tage, Skip-/Statusperioden und Daily-Health 730 Tage, Cardio/Glukose/Blutdruck/Temperatur 365 Tage, Wasser 90 Tage und Adaptive-TDEE 730 Tage. Bei großen Pending-Basen werden unveränderte Zeilen nur per ID-Manifest gespeichert und beim Laden rekonstruiert. Alte lokal geänderte oder noch nicht gesyncte Zeilen bleiben bis zum erfolgreichen Sync geschützt; Supabase/Export behalten die vollständige Historie.
...
- **Cloudflare-Build-Vertrag** (im Cloudflare-Dashboard konfiguriert, hier nur dokumentiert, da `wrangler.json` kein `build.command`-Feld nutzt): Build command = `npm run build`, Build output directory = `dist`. `npm run build` ruft `node tools/build.cjs` auf (siehe `package.json`); CI führt denselben Befehl in `check.yml` aus, bevor `tools/check-build.cjs` die Ausgabe validiert, das Preview kann also nie einen Build-Befehl bekommen, der lokal ungetestet ist. Ändert sich der Cloudflare-Dashboard-Wert, diese Zeile im Gleichschritt nachziehen.
