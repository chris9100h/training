...
- **localStorage-Keys** (einige Settings liegen parallel im localStorage für schnellen Zugriff vor dem Store-Load; bestehende Keys konsistent halten):
  ...
  - `logbook-meal-parser-provider`: `'claude'` (Default) oder `'grok'`, welche Edge Function (`parse-meal` bzw. `parse-meal-grok`) "Describe a meal" liest. Toggle im Describe-a-meal-Sheet (`FoodScreen`), gleiches Vergleichs-/Debug-Prinzip wie der Label-Scanner-Toggle, kein User-Setting, per Gerät. Anders als dort kein Cross-Broadcast nötig (kein zweiter gleichzeitig gemounteter Einstiegspunkt)
