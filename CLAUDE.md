...
- **localStorage-Keys** (einige Settings liegen parallel im localStorage für schnellen Zugriff vor dem Store-Load; bestehende Keys konsistent halten):
  ...
  - `logbook-shopping-exclusions`: jsonb-Objekt `{ [foodId]: foodName }`, User-Ausschluss einzelner Shopping-List-Einträge (Tap auf eine Zeile mit `foodId` → "Exclude from shopping list"), für Foods die zwar nach Frequenz/Plan qualifizieren aber kein Grocery-Haul-Item sind (z.B. Whey, alle paar Monate woanders bestellt). `foodName` wird mitgespeichert, damit das "Excluded items"-Sheet (Link unter dem Tage-Stepper) sie auch anzeigen kann, wenn sie längst nicht mehr in der aktuellen Liste auftauchen. Restore jederzeit über dasselbe Sheet, kein User-Setting, per Gerät
