...
- **Medications** (`zane_medication_plans`/`zane_medications`/`zane_medication_schedule_slots`/`zane_medication_logs`): persönlicher, optional coach-verwalteter Medikamenten-/Vitamin-/Supplement-Tracker, architektonisch am Food-Tracker-Plan-Mode orientiert (Container → Items → Wochentags-Schedule → Log), Coach-of-Client-RLS wie Meal Plans (voller Zugriff, nicht nur selbst gepusht). Mehrere Pläne laufen bewusst gleichzeitig (kein Active-Pointer wie bei Food). `zane_medication_schedule_slots` trägt optionale `start_date`/`end_date` für echte zeitlich gestaffelte Cycles (beide null = Default/unbegrenzt). Feature-Master-Switch `zane_user_settings.meds_enabled` (Default aus), 4. Slot im Health/Water/Food-TabBar-Zyklus. Eigene Edge Function `medication-reminder` (stündlicher Cron) nach `meal-reminder`-Vorbild. Details: `docs/database.md`. Migration 0218, Reminder-Cron 0219.
