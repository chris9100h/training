-- Return the exact client reporting-week key used for coach review dismissals.
-- The existing no-argument RPC changes its composite return type, so it must
-- be dropped before it can be recreated with the additional date column.

DROP FUNCTION IF EXISTS public.get_coach_checkin_status();

CREATE FUNCTION public.get_coach_checkin_status()
