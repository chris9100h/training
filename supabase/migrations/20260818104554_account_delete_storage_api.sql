-- Account deletion: Storage objects must go through the Storage API.
--
-- Supabase rejects direct DELETE statements against storage.objects. The
-- client captures the user's own paths and removes them with
-- supabase.storage.from(...).remove(...) before calling this RPC. This
-- function therefore deletes only database metadata and social rows.
...
BEGIN
  ...
  -- Do not delete storage.objects here. Storage metadata and the underlying
  -- object are removed by the authenticated Storage API call in the client.
