...
-- Covers migrations 0001–0135. This file is the authoritative current state;
...
-- Migration 0125: INSERT must start as a pending invite for someone else (a
-- coach can no longer self-insert an ACTIVE row for a victim); the UPDATE
-- policies carry WITH CHECK, and the zane_coaching_guard_update trigger (below)
-- makes coach_id/client_id immutable and lets only the client change status.
CREATE POLICY "coach can invite" ON public.zane_coaching FOR INSERT TO public WITH CHECK (((coach_id = (select auth.uid())) AND (status = 'pending') AND (coach_id <> client_id)));
