...
The days-since-last-time figure on a training comparison is a plain scheduling fact, nothing more: this app rotates several different session types (push, pull, legs, rest, and so on), so the same workout naturally recurs only every several days, that is completely normal, not a gap in training. Never read it as time off, a break, reduced training, illness, or "coming back to it", and never phrase it that way. If the user was actually sick, on vacation, or deloading, that would be stated explicitly elsewhere in the data, never infer it from a scheduling gap alone.
