...
// estimated food log items, Qwen variant. Third interchangeable backend behind
// the same contract and the same prompt as parse-meal (Anthropic Claude, the
// long-standing default) and parse-meal-grok (xAI), mirroring how
// scan-label-qwen joins scan-label and scan-label-claude. Keep all three in
// step: a change to SYSTEM_PROMPT or the returned shape belongs in every one
// of them, or the toggle stops comparing the same thing.
//
...
// SYSTEM_PROMPT. Vision runs through the same chat completion call as the
// text-only path, no separate request or model.
//
...
// trusted as a separate number from the model, same rule search-foods applies
...
//
// Needs QWEN_API_KEY (the same secret scan-label-qwen uses), plus the same two
// optional overrides for the reasons documented there: QWEN_URL (default is
...
// Alibaba Cloud Model Studio's OpenAI-compatible endpoint, same dialect as the
// xAI call in parse-meal-grok, which is why that file was the template.
...
// so both bounds are generous.
const MAX_PREVIOUS_ITEMS = 50;
const MAX_ITEM_NAME_CHARS = 200;

// parse-meal-grok caps completions at 2000, enough for the visible self-check
// arithmetic plus the JSON. That budget does not survive thinking being on:
// reasoning tokens are billed and counted as completion tokens, so a long
// deliberation would eat the allowance and truncate the JSON object itself,
// which surfaces as a bogus "could not find any food" rather than as an
// error. The ceiling only bounds what may be generated, it is not a
// reservation, so the thinking budget is generous and the no-think budget
...
// parse-meal, shared 'meal_parse' kind: switching providers must not multiply
...
// Identical to parse-meal's own SYSTEM_PROMPT: all three providers must be
...
// parse-meal/parse-meal-grok/scan-label.
...
// Best-effort short reason from the provider's error body, same as scan-label.
...
// Same backstop as parse-meal/parse-meal-grok/ai-daily-summary: CLAUDE.md's
...
Deno.serve(async (req) => {
  ...
  if (!apiKey) return json({ error: 'Meal parsing is not set up yet (missing QWEN_API_KEY).' }, 503);
  const model = (Deno.env.get('QWEN_MODEL') ?? '').trim() || DEFAULT_MODEL;
  const url = (Deno.env.get('QWEN_URL') ?? '').trim() || DEFAULT_URL;

  // See the thinking policy at the top of this file: on is the right default
  // here, so the default path sends no flag at all and inherits the model's.
  // Turning it off uses the same two switches scan-label-qwen documents, a
  // body flag plus the in-prompt equivalent, because the front doors selling
  // this model honour different ones.
  const noThink = (Deno.env.get('QWEN_NO_THINK') ?? '').trim() === '1';
  const userText = buildUserText(description, !!image, previousItems);
  const promptText = noThink ? `${userText}\n/no_think` : userText;
  ...
  try {
      ...
      body: JSON.stringify({
        ...
        messages: [
          { role: 'system', content: SYSTEM_PROMPT },
          {
            ...
            content: image
              ? [{ type: 'text', text: promptText }, { type: 'image_url', image_url: { url: `data:${mimeType};base64,${image}`, detail: 'high' } }]
              : promptText,
          },
        ],
      }),
    ...
    console.error('[parse-meal-qwen] fetch error:', e);
    return json({ error: 'Could not reach the meal estimator. Try again.' }, 502);
  }

  if (!resp.ok) {
    const detail = await resp.text().catch(() => '');
    console.error('[parse-meal-qwen] error', resp.status, detail);
    const reason = errReason(detail);
    return json({ error: `Meal estimator failed (${resp.status})${reason ? ': ' + reason : ''}` }, 502);
  }

  const data = await resp.json().catch(() => null);
  // Qwen keeps reasoning in a separate `reasoning_content` field, so `content`
  // holds the answer alone whether thinking ran or not. The prompt's visible
  // self-check arithmetic is deliberately brace-free, so extractJson's first
  // `{` is the JSON object either way.
