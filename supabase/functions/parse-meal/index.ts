...
function recipeKcalBand(name: string): RecipeKcalBand | null {
  ...
  if (/\b(flour|mehl)\b/.test(normalized)) return { min: 3.3, max: 4.0 };
  if (/\b(cocoa powder|cacao powder|kakaopulver|kakao)\b/.test(normalized)) return { min: 2.0, max: 3.5 };
  ...
  if (/\b(dark chocolate|milk chocolate|white chocolate|baking chocolate|chocolate chips|chocolate bar|chocolate|dunkle schokolade|schokolade|couverture|kuvertuere)\b/.test(normalized)) return { min: 4.5, max: 6.5 };
  if (/\b(mascarpone)\b/.test(normalized)) return { min: 4.0, max: 5.2 };
