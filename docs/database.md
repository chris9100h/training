...
- `planned_technique` (text, nullable) + `planned_technique_scope` (text, nullable): **planned intensity technique** for this exercise, chosen in the plan/template item editor so it auto-arms live during training instead of relying on the client to pick it. `planned_technique` is `null` (none) or one of the `zane_sets.technique` values (`'drop'` | `'myorep'` | `'myorep_match'` | `'amrap_variations'` | `'lengthened_partial'` | `'weighted_stretch'`); `planned_technique_scope` is `'last'` (last working set only) or `'all'` (every working set), `null` when no technique. Copied from the schedule day item / template at session start (like the other `planned_*` columns) so a later plan edit does not change an in-progress session; carried in the `zane_schedules.days`/`zane_workout_templates.exercises` JSONB as `plannedTechnique`/`plannedTechniqueScope` passthrough. Store fields `plannedTechnique`/`plannedTechniqueScope`. Surfaced by `zane_entries_json`. Migration 0157.
