...
- `meso_recap` (jsonb, nullable): durable per-session snapshot of the autoregulation / mesocycle feedback given and the weight/set bumps or cuts earned, so the session detail screen (`SessionDetailScreen`, „Feedback recap" button) can show it long after the fact and across devices. Written at session finish in `screens-train.jsx` (`buildMesoRecap`), previously only in device localStorage. Shape: `{ loadOnly, meso, week, weeks, unit, groups: [{ muscle, general: [{title,sub}], joint: [{title,sub}] }], gains: [{ name, weightDelta, setDelta }], raw }` (app-defined, not queried server-side; `week`/`weeks` are the mesocycle position and block length, both `null` outside a mesocycle plan). `raw` (present only when feedback was given) is the durable copy of the per-question answer records that powers post-hoc feedback editing: `{ answers: { soreness: {muscle: rec}, joint: {exId: rec}, volume: {muscle: rec} }, negOwner, frozen, dayId }`. Store field `mesoRecap`, only mapped onto the row when set. Part of the user backup (round-trips via `sessionToRow`). Migration 0166.
  - **Load-only weight-feel is per exercise (shape contract, read carefully before touching the recap edit path or the meso gates).** In `autoregLoadOnly` plans the "how did the weight feel" question is asked **per exercise**, folded into that exercise's joint record as `joint[exId].weight` (a `not_enough`/`just_right`/`pushed`/`too_much` code), and the per-muscle `volume[muscle]` record is kept **pump-only** (`pump` set, `volume` absent). The weight-bump gate is therefore keyed by `exId` (`weightOk` in `LB.mesoGateSetsFromAnswers`), not by muscle. Soreness stays per muscle. **Backward-compat:** sessions finished (or live-DB backfilled) before this change carry the load-only weight answer **per muscle** in `volume[muscle].volume` instead, with no `joint[exId].weight`. The **weight-bump gate** resolves this **per exercise** and never unions the two sources: `LB.mesoGateSetsFromAnswers` (store.js) and its live-capture mirror `mesoBoostSetsInitRef` (screens-train.jsx) take each exId's own `joint[exId].weight` when it has one (an explicit answer is never overridden), and for any exId **without** one fall back to that exId's muscle `volume[muscle].volume`. This one rule covers all three shapes: fully-new (every joint rec weighted, `volume[muscle]` pump-only so the fallback is inert), fully-old (no joint weights, the fallback drives everything), and a **mixed** raw whose feedback straddled a mid-recap client update (some exercises answered per-exercise, some still per-muscle). The **display/edit** builders `fbEditRows`/`fbGroupsForStore` (screens-lib.jsx) instead use a session-level probe (`perExWeight = loadOnly && Object.values(joint).some(r => r && 'weight' in r)`) to keep one consistent look within a session (every joint row edits weight, or none does); this only drives labels, not the gate. Non-load-only modes (Volume+Load, Mesocycle) are unchanged: joint is joint-only, `volume[muscle]` carries both pump and the workload answer.
...
- **`sync_meso_states_batch(p_states jsonb)`** → `void` (SECURITY INVOKER): Batch-Upsert für Meso-States mit `updated_at`-Staleness-Guard, damit zwei Geräte, die denselben Meso-Plan trainieren, sich nicht still gegenseitig `deltas`/`joint_flags`/`weight_boosts`/`growth_counts`/`rep_miss_counts` clobbern. Migration 0122 (`growth_counts` ergänzt in 0130, `started_at` in 0138: beim Update COALESCEd, damit ein älterer Client, der es weglässt, den Anker nicht nullen kann; `rep_miss_counts` ergänzt in 0165).
